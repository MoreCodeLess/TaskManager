...
function deleteSelectedTasks() {
    ...
    const savedTasks = JSON.parse(localStorage.getItem("tasks")) || [];
    const selectedTasks = [];
    for(let i = 0; i < tasks.length; i++)if (tasks[i].checked) selectedTasks.push(savedTasks[i]);
...
function loadDashTasks() {
    ...
    tasksToInject.forEach((element, index)=>{
        ...
        tasksHTML += `
      <div class="task">
        <input class="task-item" name="task" type="checkbox" id="item-${index + 1}">
    ...
    const sortedTasks = tasksToInject.sort((a, b)=>new Date(b.date) - new Date(a.date));
    ...
    topThreeTasks.forEach((element, index)=>{
        console.log(element);
        schedulesHTML += `
        <div class="task-box ${element.category == "progress" ? "blue" : element.category == "approved" ? "yellow" : "red"}">
